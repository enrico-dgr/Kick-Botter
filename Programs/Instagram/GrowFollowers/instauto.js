...
const Instauto = async (db, browser, options) => {
  ...
  async function checkReachedFollowedUserDayLimit() {
    const reachedFollowedUserDayLimit =
      getNumFollowedUsersThisTimeUnit(dayMs) >= maxFollowsPerDay;
  ...
  async function checkReachedFollowedUserHourLimit() {
    const hasReachedFollowedUserHourLimit =
      getNumFollowedUsersThisTimeUnit(hourMs) >= maxFollowsPerHour;
  ...
  async function findFollowButton() {
    ...
    const elementHandles2 = await page.$x(
  ...
  async function findUnfollowButton() {
    ...
    const elementHandles3 = await page.$x(
  ...
  async function getFollowersOrFollowing({
    ...
    getFollowers = false,
    maxPages,
    shouldProceed: shouldProceedArg,
  }) {
    ...
    const shouldProceed = () => {
      ...
      if (shouldProceedArg)
        return isBelowMaxPages && shouldProceedArg(outUsers);
      return isBelowMaxPages;
    };

    while (shouldProceed()) {
      const url = `${
        getFollowers ? followersUrl : followingUrl
  ...
  async function likeCurrentUserImages({
    username,
    likeImagesMin,
    likeImagesMax,
  } = {}) {
  ...
  async function followUserFollowers(
    username,
    {
      maxFollowsPerUser = 5,
      skipPrivate = false,
    ...
    const shouldProceed = (usersSoFar) =>
      usersSoFar.filter((u) => !getPrevFollowedUser(u)).length <
      maxFollowsPerUser + 5; // 5 is just a margin
    ...
    for (const follower of followers) {
      ...
      try {
        ...
        if (isPrivate && skipPrivate) {
          ...
          (followUserMinFollowing != null &&
            followsCount < followUserMinFollowing)
        ) {
          logger.log(
  ...
  async function followUsersFollowers(
    ...
    const maxFollowsPerUser =
      Math.floor(maxFollowsTotal / usersToFollowFollowersOfSliced.length) + 1;
  ...
  async function safelyUnfollowUserList(usersToUnfollow, limit) {
    ...
    for (const username of usersToUnfollow) {
      try {
        ...
        i += 1;
        logger.log(
          `Have now unfollowed ${i} users of total ${usersToUnfollow.length}`
        );
  ...
  async function unfollowOldFollowed({ ageInDays, limit } = {}) {
    ...
    logger.log(
      `Unfollowing currently followed users who were auto-followed more than ${ageInDays} days ago...`
    );
    ...
    const usersToUnfollow = allFollowing
      .filter(
        (u) =>
          getPrevFollowedUser(u) &&
          !excludeUsers.includes(u) &&
          (new Date().getTime() - getPrevFollowedUser(u).time) /
            (1000 * 60 * 60 * 24) >
            ageInDays
      )
      .slice(0, limit);
